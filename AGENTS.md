...
- The current project directory is `/n/holylfs05/LABS/informatics/Lab/projects/gwct/phyloacc/PhyloAcc`.
...
- This project assumes the use of a conda environment named `phyloacc-test` that is activated at the beginning of every session. Always try to activate this environment at the beginning of every session.
- If the `phyloacc-test` environment cannot be activated, run all commands except `echo`, `ls`, `cd`, `grep`, `sed`, `cat`, and `awk` with the conda environment by using `conda run -n phyloacc-test <command>`.
